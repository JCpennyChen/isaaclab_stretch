# This file is from CuRobo's example scripts for transforming Robot URDF files to USD format.
...
def save_usd():
    ...
    import_config.fix_base = not args.mobile_base
    ...
    robot_config = load_yaml(join_path(get_robot_configs_path_own_folder(), args.robot))
    urdf_path = join_path(get_assets_path_own_folder(), robot_config["robot_cfg"]["kinematics"]["urdf_path"])
    asset_path = join_path(
        get_assets_path_own_folder(), robot_config["robot_cfg"]["kinematics"]["asset_root_path"]
    )
    urdf_interface = _urdf.acquire_urdf_interface()
    full_path = join_path(get_assets_path_own_folder(), robot_config["robot_cfg"]["kinematics"]["urdf_path"])
    ...
    save_path = join_path(get_assets_path_own_folder(), robot_config["robot_cfg"]["kinematics"]["usd_path"])
    print("save_path: ", save_path)
    continue_signal = input("Press Y to continue and press N if this is not the correct path.")
    if continue_signal.lower() == "y":
